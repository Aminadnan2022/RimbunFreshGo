-- Phase 4B.1: canonical atomic order-placement backend.
-- This RPC is CANONICAL-ONLY. It does not read or write legacy public."Orders".
-- The current frontend (CheckoutPage.tsx / OrderContext.tsx) is NOT modified
-- and continues to use legacy Orders until Phase 4B.2.
--
-- Money is never trusted from the client. unit_selling_price, unit_cost_price,
-- product_version_id, combo_version_id, delivery_fee, and payment/delivery
-- configuration are all resolved server-side from canonical effective-dated
-- state. Client input only selects WHAT (product/combo ids, quantities,
-- requested delivery method/date/time/zone, preparation answers) — never HOW
-- MUCH or WHICH historical version.

-- -----------------------------------------------------------------------------
-- 1. Concurrency-safe order number generation
-- -----------------------------------------------------------------------------
...
AS $$
  SELECT 'RFG-' || to_char(now() AT TIME ZONE 'Asia/Kuala_Lumpur', 'YYYYMMDD')
...
-- -----------------------------------------------------------------------------
-- 2. place_sales_order — one atomic canonical checkout transaction
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -----------------------------------------------------------------------------
  -- Finalise order-level totals now that every line is known.
  -----------------------------------------------------------------------------
