-- Phase 4B.2 corrective migration: place_sales_order projection guard.
--
-- ROOT CAUSE (confirmed by direct inspection of the applied 20260918000000
-- migration, not a prior summary):
--   place_sales_order() executes exactly two UPDATE statements:
--     1. UPDATE public.sales_order_lines SET final_supplier_cost = ...
--        (fixed-price combo aggregate cost, inside the combo item loop)
--     2. UPDATE public.sales_orders SET requires_supplier_finalisation,
--        price_status, estimated_subtotal, estimated_total, final_subtotal,
--        final_total, price_finalised_at, subtotal, total ... (the header
--        projection update, once per order, after all lines are resolved)
--   Statement (1) wrapped itself in freshgo.canonical_operation =
--   'price_finalisation' — the wrong operation, since this is creation-time
--   work, not later supplier/admin finalisation. Statement (2) had NO
--   canonical_operation set at all, so it only ever succeeded by accident
--   (only when a preceding combo branch happened to leak 'price_finalisation'
--   forward in the same transaction). For any order without that leak
--   (e.g. a plain fixed-price product, or a whole_fish_by_weight product with
--   no combo line), the header UPDATE was rejected by the append-only trigger.
--   Separately, phase4a_validate_order_transitions() also requires
--   v_operation = 'price_finalisation' for the estimated -> final price_status
--   transition, which is likewise wrong for the at-creation case.
--
-- FIX: introduce a narrowly scoped 'order_creation' operation, accepted ONLY
-- for the exact fields place_sales_order actually sets, on ONLY the two
-- tables it actually updates. Real supplier/admin finalisation
-- (finalize_sales_order_pricing) keeps using 'price_finalisation' unchanged.
-- The operation is set immediately around each controlled UPDATE and cleared
-- straight after, so it never leaks to any other statement in the
-- transaction (including the sales_order_events INSERT that follows).

-- -----------------------------------------------------------------------------
-- 1. Extend the append-only mutation guard with order_creation.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'sales_order_lines' AND v_operation = 'order_creation'
     AND (to_jsonb(NEW) - ARRAY['final_supplier_cost'])
         IS NOT DISTINCT FROM
         (to_jsonb(OLD) - ARRAY['final_supplier_cost']) THEN
    RETURN NEW;
...
-- -----------------------------------------------------------------------------
-- 2. Extend the price_status transition guard: the at-creation
--    estimated -> final transition (no supplier finalisation required) is a
--    creation-time fact, not a later admin/supplier finalisation event.
--    payment_status transition rules are untouched.
-- -----------------------------------------------------------------------------
...
BEGIN
  IF NEW.price_status IS DISTINCT FROM OLD.price_status
     AND NOT (OLD.price_status = 'estimated' AND NEW.price_status = 'final'
              AND v_operation IN ('price_finalisation', 'order_creation')) THEN
...
-- -----------------------------------------------------------------------------
-- 3. place_sales_order — identical to the applied version except the two
--    controlled UPDATEs now run under 'order_creation' (not
--    'price_finalisation'), and the operation is cleared immediately after
--    each one so it cannot leak into any later statement in the transaction.
-- -----------------------------------------------------------------------------
      ...
      IF v_answer_unit IS NOT NULL THEN
        IF v_answer_component IS NOT NULL THEN
          SELECT id INTO v_answer_component_unit
           ...
           WHERE sales_order_line_component_id = v_component_row.id AND unit_number = v_answer_unit;
