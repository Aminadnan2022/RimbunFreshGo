-- A combo component stores its configured per-combo quantity, while the parent
-- sales-order line stores how many copies of the combo were ordered. Materialise
-- preparation units for every physical component across every ordered combo.
...
BEGIN
  ...
  IF NEW.quantity <> trunc(NEW.quantity) OR NEW.quantity <= 0
    ...
    RAISE EXCEPTION
      'Combo component % requires positive whole component and combo quantities (component %, combo %).',
