-- Prevent physical-unit projection from running for a Customer Choice
-- alternative whose component INSERT was suppressed by the BEFORE trigger.
--
-- The keyed public RPC and staged-payment wrapper remain unchanged. This
-- replaces only their canonical unkeyed implementation, filters the immutable
-- combo recipe to fixed plus selected version items before projection, and
-- assigns dense component numbers from that filtered recipe.
...
BEGIN
  ...
  LOOP
    ...
    IF v_product_id IS NOT NULL THEN
      ...
      -- Canonical checkout must never freeze an unresolved supplier or
      -- zero/missing supplier cost into an immutable order snapshot.
      IF v_supplier_id IS NULL
         OR v_unit_cost_price IS NULL
         OR v_unit_cost_price <= 0
      THEN
        RAISE EXCEPTION
          'Canonical supplier price is required for product %',
          v_product_id
          USING ERRCODE = '23514';
      END IF;
               ...
               AND (
                 ...
                 OR EXISTS (
                   SELECT 1
                    ...
                    WHERE selected->>'choice_group_key' = ci.choice_group_key
                      AND (
                        selected->>'combo_item_id' = ci.id::text
                        OR selected->>'combo_item_id' = ci.source_combo_item_id::text
                        OR EXISTS (
                          SELECT 1
                            FROM public.combo_items current_item
                           WHERE current_item.id::text = selected->>'combo_item_id'
                             AND current_item.combo_id = v_combo_version.combo_id
                             AND current_item.choice_group_key = ci.choice_group_key
                             AND current_item.product_id = ci.product_id
                        )
                      )
                 )
               )
          ) selected_items
         ORDER BY selected_items.display_order
      LOOP
        ...
        -- Combo components are supplier-owned fulfilment records, so each
        -- component must also have a resolved supplier and positive cost.
        IF v_component_supplier_price.supplier_id IS NULL
           OR v_component_supplier_price.cost_price IS NULL
           OR v_component_supplier_price.cost_price <= 0
        THEN
          RAISE EXCEPTION
            'Canonical supplier price is required for combo component product %',
            v_combo_item.product_id
            USING ERRCODE = '23514';
        END IF;

        v_component_estimated_weight := NULLIF(
          v_item -> 'component_estimated_weights' ->> v_combo_item.projected_component_number::text, ''
        )::numeric;
        ...
        INSERT INTO public.sales_order_line_components (
          ...
          v_sales_order_line_id, v_combo_item.combo_version_item_id, v_combo_item.projected_component_number,
        ...
        IF v_component_id IS NULL THEN
          RAISE EXCEPTION
            'Line %: selected combo component % was not projected.',
            v_line_number, v_combo_item.combo_version_item_id;
        END IF;

        -- The AFTER INSERT materializer owns component-unit creation. It runs
        -- only after this component has a real primary key and covers both
        -- whole_fish_by_weight and schemas with physical-unit preparation.
        -- Keeping unit creation in one place also avoids duplicate rows.
      END LOOP;

      -- Combo customer revenue (parent line_total/finalised_at) is already
      -- final regardless of component state. Supplier cost and physical-unit
      -- weighing are separate operational concerns and must never change the
      -- customer-price finality of a fixed-price combo line. If every
      -- component's cost is already fully known (fixed_quantity only), freeze
      -- the aggregate now; otherwise leave component final costs open for the
      -- supplier workflow without changing the order's customer price_status.
...
BEGIN
  ...
  IF NEW.quantity <> trunc(NEW.quantity) OR NEW.quantity <= 0 THEN
    RAISE EXCEPTION 'Combo component % requires physical units but quantity % is not a positive whole count.', NEW.component_number, NEW.quantity;
  END IF;
  ...
  v_estimated_unit_weight_kg := CASE
    WHEN v_version.ordering_mode = 'whole_fish_by_weight' AND NEW.estimated_weight_kg IS NOT NULL
      THEN round(NEW.estimated_weight_kg / v_unit_count, 3)
