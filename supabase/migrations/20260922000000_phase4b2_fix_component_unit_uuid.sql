-- Phase 4B.2 corrective migration: component-unit answer column type fix.
--
-- OFFENDING SQL (in the applied 20260921000000 version of place_sales_order):
--
...
--     ..., sales_order_line_component_unit_id, ...
--   ) VALUES (
--     ..., v_answer_component_unit, ...
--   );
--
-- ROOT CAUSE: v_answer_component_unit is declared as integer, but it is
-- assigned the resolved sales_order_line_component_units.id (a uuid primary
-- key), then inserted straight into sales_order_preparation_answers
-- .sales_order_line_component_unit_id (also uuid). Because the variable's
-- declared type is integer, the INSERT's parameter for that column is typed
-- as integer, producing:
--   "column \"sales_order_line_component_unit_id\" is of type uuid but
--    expression is of type integer"
-- This only affects combo physical-unit preparation answers (component_number
-- + unit_number); normal-product units (v_unit_id) and combo line-scope
-- answers (v_answer_component_id) were already correctly typed uuid.
--
-- FIX: retype v_answer_component_unit as uuid, matching what it actually
-- holds (a sales_order_line_component_units.id). No other logic changes.
-- Resulting behavior per preparation-answer row:
--   * Normal product, unit-scoped: sales_order_line_unit_id = resolved uuid;
--     sales_order_line_component_id = NULL; sales_order_line_component_unit_id = NULL.
--   * Combo component, line-scope: sales_order_line_component_id = resolved
--     uuid; sales_order_line_component_unit_id = NULL.
--   * Combo component, physical-unit: sales_order_line_component_id +
--     sales_order_line_component_unit_id both resolved uuids (never the raw
--     integer unit_number).
--
-- Everything else is byte-for-byte identical to the applied 20260921000000
-- version.
