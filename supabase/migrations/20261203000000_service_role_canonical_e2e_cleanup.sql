...
BEGIN
  ...
  CREATE TEMP TABLE e2e_cleanup_target_orders (id uuid PRIMARY KEY) ON COMMIT DROP;
  INSERT INTO e2e_cleanup_target_orders SELECT DISTINCT unnest(COALESCE(p_order_ids, ARRAY[]::uuid[]));
  CREATE TEMP TABLE e2e_cleanup_target_batches (id uuid PRIMARY KEY) ON COMMIT DROP;
  INSERT INTO e2e_cleanup_target_batches SELECT DISTINCT unnest(COALESCE(p_batch_ids, ARRAY[]::uuid[]));
  CREATE TEMP TABLE e2e_cleanup_target_notifications (id uuid PRIMARY KEY) ON COMMIT DROP;
  INSERT INTO e2e_cleanup_target_notifications
  SELECT n.id FROM public.notifications AS n
  JOIN e2e_cleanup_target_orders AS o ON o.id = n.sales_order_id;

  DELETE FROM public.checkout_payment_receipt_staging
  WHERE consumed_sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.web_push_delivery_attempts WHERE job_id IN (
    SELECT id FROM public.web_push_delivery_jobs
    WHERE notification_id IN (SELECT id FROM e2e_cleanup_target_notifications));
  ...
  WHERE notification_id IN (SELECT id FROM e2e_cleanup_target_notifications);
  DELETE FROM public.notifications WHERE id IN (SELECT id FROM e2e_cleanup_target_notifications);
  ...
  DELETE FROM public.canonical_delivery_proofs WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.canonical_sales_order_deliveries WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.canonical_supplier_delivery_batch_orders WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.canonical_supplier_delivery_batches WHERE id IN (SELECT id FROM e2e_cleanup_target_batches);
  GET DIAGNOSTICS v_batch_count = ROW_COUNT;
  DELETE FROM public.sales_order_supplier_fulfilments WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.payment_reminder_attempts WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.sales_order_payment_receipts WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.sales_order_preparation_answers WHERE sales_order_line_id IN (
    SELECT id FROM public.sales_order_lines WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders));
  DELETE FROM public.sales_order_line_component_units WHERE sales_order_line_component_id IN (
    ...
    WHERE l.sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders));
  ...
  DELETE FROM public.sales_order_line_units WHERE sales_order_line_id IN (
    SELECT id FROM public.sales_order_lines WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders));
  DELETE FROM public.sales_order_adjustments WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.sales_order_events WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.sales_order_checkout_idempotency WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.sales_order_lines WHERE sales_order_id IN (SELECT id FROM e2e_cleanup_target_orders);
  DELETE FROM public.sales_orders WHERE id IN (SELECT id FROM e2e_cleanup_target_orders);
