-- Keep the editable Product row and the immutable checkout catalogue aligned.
-- Admin edits remain convenient, while previously published versions and
...
BEGIN
  ...
  -- Resolve the preparation schema from customer-ordering semantics. Piece
  -- fish retain per-fish preparation; weighted fish use line preparation.
  v_schema_code := CASE
