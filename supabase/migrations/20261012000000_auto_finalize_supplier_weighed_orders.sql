-- Finalise canonical pricing as soon as the final required supplier measurement
-- is recorded.  The calculation remains server-side and uses only the frozen
-- checkout prices/costs; this does not accept a supplier-provided total.
...
BEGIN
  -- A supplier may only finalise an order after all canonical measurements are
  -- present, and only where that supplier owns at least one order line/component.
  -- The calculations below still validate completeness atomically.
  ...
  SELECT * INTO v_order FROM public.sales_orders WHERE id = p_sales_order_id AND status <> 'cancelled' FOR UPDATE;
  ...
  LOOP
    ...
    v_final_supplier_cost := CASE WHEN v_line.unit_cost_price IS NULL THEN NULL ELSE round(v_line.unit_cost_price * v_actual_weight, 2) END;
    UPDATE public.sales_order_lines
       SET actual_weight_kg = v_actual_weight, final_line_total = v_final_line_total,
           final_supplier_cost = v_final_supplier_cost, finalised_at = now(), line_total = v_final_line_total
     WHERE id = v_line.id;
  END LOOP;
  ...
  LOOP
    IF v_component.ordering_mode = 'whole_fish_by_weight' THEN
      ...
      IF v_unit_count = 0 OR v_unit_present < v_unit_count THEN RAISE EXCEPTION 'Component %: missing actual weight for one or more physical units.', v_component.component_number; END IF;
    ELSE
    ...
    UPDATE public.sales_order_line_components
       SET actual_weight_kg = v_actual_weight, final_supplier_cost = v_final_supplier_cost, finalised_at = now()
     WHERE id = v_component.id;
  END LOOP;

  FOR v_line IN SELECT * FROM public.sales_order_lines WHERE sales_order_id = p_sales_order_id AND item_kind = 'combo' AND final_supplier_cost IS NULL LOOP
    SELECT sum(final_supplier_cost) INTO v_combo_cost_sum FROM public.sales_order_line_components WHERE sales_order_line_id = v_line.id;
    UPDATE public.sales_order_lines SET final_supplier_cost = v_combo_cost_sum WHERE id = v_line.id;
  END LOOP;

  SELECT COALESCE(sum(line_total), 0) INTO v_subtotal FROM public.sales_order_lines WHERE sales_order_id = p_sales_order_id;
  ...
  VALUES (p_sales_order_id, 'price_finalised', auth.uid(), jsonb_build_object('final_subtotal', v_subtotal, 'final_total', v_subtotal + v_order.delivery_fee));
...
AS $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.sales_orders WHERE id = p_sales_order_id AND price_status = 'final') THEN RETURN; END IF;
...
BEGIN
  ...
  IF v_line.final_line_total IS NOT NULL THEN RAISE EXCEPTION 'This line is already finalised.'; END IF;
  PERFORM set_config('freshgo.canonical_operation', 'price_finalisation', true);
...
BEGIN
  ...
  IF NOT (public.is_admin() OR public.is_supplier_for_sales_order_line(p_sales_order_line_id)) THEN RAISE EXCEPTION 'Not authorized for this order line.'; END IF;
...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE v_component public.sales_order_line_components%ROWTYPE; v_order_id uuid;
BEGIN
  ...
  IF NOT (public.is_admin() OR public.is_supplier_for_sales_order_line_component(p_sales_order_line_component_id)) THEN RAISE EXCEPTION 'Not authorized for this combo component.'; END IF;
  ...
  IF p_actual_weight_kg IS NULL OR p_actual_weight_kg < 0 THEN RAISE EXCEPTION 'Invalid actual weight.'; END IF;
  IF v_component.final_supplier_cost IS NOT NULL THEN RAISE EXCEPTION 'This component is already finalised.'; END IF;
  SELECT sales_order_id INTO v_order_id FROM public.sales_order_lines WHERE id = v_component.sales_order_line_id;
  PERFORM set_config('freshgo.canonical_operation', 'price_finalisation', true);
  ...
  PERFORM public.phase4c6_finalize_if_measurements_complete(v_order_id);
END;
$$;
...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE v_unit public.sales_order_line_component_units%ROWTYPE; v_component public.sales_order_line_components%ROWTYPE; v_order_id uuid;
BEGIN
...
REVOKE EXECUTE ON FUNCTION public.phase4c6_finalize_if_measurements_complete(uuid) FROM PUBLIC;
