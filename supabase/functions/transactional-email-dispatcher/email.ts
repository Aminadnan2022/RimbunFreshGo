export const CUSTOMER_EMAIL_TYPES = ["order_payment_submitted", "price_finalised", "payment_confirmed", "payment_receipt_rejected", "ready_for_delivery", "out_for_delivery", "order_delivered", "order_cancelled"] as const;
...
export type TransactionalEmailProjection = {
  notification_type: CustomerEmailType; order_number: string; final_total: number | string | null;
...
const paymentLabel = (type: CustomerEmailType, status: string | null) => type === "order_payment_submitted" ? "Awaiting receipt verification" : type === "price_finalised" ? "Payment required" : type === "payment_confirmed" || status === "paid" ? "Confirmed" : type === "payment_receipt_rejected" ? "Replacement receipt required" : null;
...
export function renderTransactionalEmail(notification: TransactionalNotification, data: TransactionalEmailProjection, options: { appBaseUrl?: string } = {}) {
  ...
  const amount = type === "price_finalised" || type === "payment_confirmed" ? amountLabel(data.final_total, data.currency_code) : null;
  ...
  const rows: string[][] = [["Order", order], ...(payment ? [["Payment", payment]] : []), ...(amount ? [[type === "payment_confirmed" ? "Amount paid" : "Final amount", amount]] : []), ...(delivery && dateLabel(data.delivery_date) ? [["Delivery date", dateLabel(data.delivery_date)!]] : []), ...(delivery && data.delivery_window ? [["Delivery window", data.delivery_window.slice(0, 80)]] : []), ...(delivery && data.delivery_area ? [["Delivery area / point", data.delivery_area.slice(0, 120)]] : [])];
