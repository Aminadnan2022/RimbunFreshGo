...
import { concisePreparationText, conciseReviewLabel } from '../lib/checkoutReview';
...
const Preparation = () => (
  ...
  const Review = () => <div className="card p-5 sm:p-8 space-y-5"><h2 className="font-semibold text-lg">{t('checkout.reviewTitle')}</h2>{cart.items.map((item, index) => { const lineKey = `line-${index}`; const lineTargets = targets.filter((target) => target.lineKey === lineKey); return <div key={index} className="border-b pb-3 text-sm text-gray-600">{item.isCombo && item.comboItems?.length ? <div className="space-y-2">{item.comboItems.map((component, componentIndex) => { const componentNumber = component.componentNumber ?? componentIndex + 1; const target = lineTargets.find((candidate) => candidate.componentNumber === componentNumber); const quantity = comboComponentQuantity(component, item.quantity); const usefulQuantity = component.sellingUnit === 'kg' || component.pricingType === 'per_kg' ? quantity : ''; const rows = !target ? [] : target.quantity > 1 ? Array.from({ length: target.quantity }, (_, unit) => ({ label: conciseReviewLabel(target, unit), preparation: reviewText(target, unit) })) : [{ label: component.name, preparation: reviewText(target, 0) || reviewText(target, null) }]; return <div key={`${componentNumber}-${component.productId}`}>{rows.length ? rows.map((row, unit) => <p key={unit}><span className="font-medium text-gray-900">{row.label}</span> — {[usefulQuantity, row.preparation].filter(Boolean).join(' · ')}</p>) : <p><span className="font-medium text-gray-900">{component.name}</span> — {quantity}</p>}</div>; })}</div> : (() => { const target = lineTargets[0]; const quantity = weighted(item) ? `${item.estimatedWeight ?? 0}kg` : t('checkout.qty', { count: item.quantity }); const usefulQuantity = weighted(item) ? quantity : ''; const rows = !target ? [] : target.quantity > 1 ? Array.from({ length: target.quantity }, (_, unit) => ({ label: conciseReviewLabel(target, unit), preparation: reviewText(target, unit) })) : [{ label: item.name, preparation: reviewText(target, 0) || reviewText(target, null) }]; return rows.length ? <div className="space-y-1">{rows.map((row, unit) => <p key={unit}><span className="font-medium text-gray-900">{row.label}</span> — {[usefulQuantity, row.preparation].filter(Boolean).join(' · ')}</p>)}</div> : <p><span className="font-medium text-gray-900">{item.name}</span> — {quantity}</p>; })()}</div>; })}<div className="rounded-xl bg-cream-50 p-4 text-sm"><p className="font-semibold">{details.name}</p><p>{details.houseUnit}, {details.deliveryPointName}</p><p>{details.phone} · {details.email}</p><p>{nextDate(deliveryDay!)} · {config.time}</p></div><Totals/><p className="text-xs text-amber-700">{t('checkout.finalPriceAfterWeighing')}</p><div className="flex gap-3"><button className="btn-secondary flex-1" onClick={() => setStep('preparation')}>{t('checkout.back')}</button><button className="btn-primary flex-1" onClick={() => setStep('payment')}>{t('checkout.continueToPayment')}</button></div></div>;
