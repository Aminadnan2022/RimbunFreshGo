...
import { Camera, CheckCircle2, ChevronLeft, ChevronRight, Info, Lock, Upload } from 'lucide-react';
...
const Preparation = () => (
  ...
  const ReceiptPicker = () => <div className="flex flex-wrap gap-3"><label className="btn-secondary inline-flex cursor-pointer items-center gap-2"><Upload size={16}/>Choose file<input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" disabled={receiptUploading || placing} onChange={(event) => selectReceiptFile(event.target.files?.[0] ?? null)} className="sr-only"/></label><label className="btn-secondary inline-flex cursor-pointer items-center gap-2"><Camera size={16}/>Use camera<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" disabled={receiptUploading || placing} onChange={(event) => selectReceiptFile(event.target.files?.[0] ?? null)} className="sr-only"/></label></div>;
