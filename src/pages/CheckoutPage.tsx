...
useEffect(() => {
  ...
  const createOrder = async () => { if (!settings.allow_customer_orders) return setPlaceError(t('checkout.validation.ordersDisabled')); if (priceFinalAtCheckout && !paymentPreview) return setPlaceError(t('payment.qrUnavailable')); if (placementLock.current) return; placementLock.current = true; setPlacing(true); setPlaceError(null); try { const idempotencyKey = checkoutAttemptKey.current ?? crypto.randomUUID(); checkoutAttemptKey.current = idempotencyKey; const request = buildCanonicalPlaceOrderRequest({ idempotencyKey, customer: details, items: cart.items, deliveryMethod, deliveryDay, instantDate, instantTime, preparationTargets: targets, preparationAnswers: answers }); if (priceFinalAtCheckout && paymentPreview) { request.p_expected_final_total = total; request.p_expected_payment_configuration_version_id = paymentPreview.configurationVersionId; } const order = await placeCanonicalOrder(request); checkoutAttemptKey.current = null; await supabase.from('customer_profiles').upsert({ id: user!.id, email_address: user!.email, full_name: details.name, phone: details.phone, apartment: details.apartment, house_unit: details.houseUnit, pickup_location: details.pickupLocation, notes: details.notes || null, updated_at: new Date().toISOString() }, { onConflict: 'id' }); clearCart(); navigate(`/order/${order.order_number}`); } catch (err) { setPlaceError(err instanceof Error ? err.message : t('checkout.validation.failedToPlaceOrder')); } finally { placementLock.current = false; setPlacing(false); } };
...
const Preparation = () => (
  ...
  const Review = () => <div className="card p-5 sm:p-8 space-y-5"><h2 className="font-semibold text-lg">{t('checkout.reviewTitle')}</h2>{cart.items.map((item, index) => { const lineKey = `line-${index}`; const lineTargets = targets.filter((target) => target.lineKey === lineKey); return <div key={index} className="border-b pb-3 text-sm text-gray-600">{item.isCombo && item.comboItems?.length ? <div className="space-y-2">{item.comboItems.map((component, componentIndex) => { const componentNumber = component.componentNumber ?? componentIndex + 1; const target = lineTargets.find((candidate) => candidate.componentNumber === componentNumber); const quantity = orderedQuantityText(component, item.quantity); const rows = !target ? [] : target.quantity > 1 ? Array.from({ length: target.quantity }, (_, unit) => ({ label: conciseReviewLabel(target, unit), preparation: reviewText(target, unit) })) : [{ label: component.name, preparation: reviewText(target, 0) || reviewText(target, null) }]; return <div key={`${componentNumber}-${component.productId}`}>{rows.length ? rows.map((row, unit) => <p key={unit}><span className="font-medium text-gray-900">{row.label}</span> — {[quantity, row.preparation].filter(Boolean).join(' · ')}</p>) : <p><span className="font-medium text-gray-900">{component.name}</span> — {quantity}</p>}</div>; })}</div> : (() => { const target = lineTargets[0]; const quantity = orderedQuantityText(item); const rows = !target ? [] : target.quantity > 1 ? Array.from({ length: target.quantity }, (_, unit) => ({ label: conciseReviewLabel(target, unit), preparation: reviewText(target, unit) })) : [{ label: item.name, preparation: reviewText(target, 0) || reviewText(target, null) }]; return rows.length ? <div className="space-y-1">{rows.map((row, unit) => <p key={unit}><span className="font-medium text-gray-900">{row.label}</span> — {[quantity, row.preparation].filter(Boolean).join(' · ')}</p>)}</div> : <p><span className="font-medium text-gray-900">{item.name}</span> — {quantity}</p>; })()}</div>; })}<div className="rounded-xl bg-cream-50 p-4 text-sm"><p className="font-semibold">{details.name}</p><p>{details.houseUnit}, {details.deliveryPointName}</p><p>{details.phone} · {details.email}</p><p>{nextDate(deliveryDay!)} · {config.time}</p></div><Totals/>{!priceFinalAtCheckout && <p className="text-xs text-amber-700">{t('checkout.finalPriceAfterWeighing')}</p>}<div className="flex gap-3"><button className="btn-secondary flex-1" onClick={() => setStep('preparation')}>{t('checkout.back')}</button><button className="btn-primary flex-1" onClick={() => setStep('payment')}>{t('checkout.continueToPayment')}</button></div></div>;
  ...
  const Payment = () => <div className="card p-5 sm:p-8 space-y-5"><h2 className="font-semibold text-lg">{t('payment.title')}</h2>{priceFinalAtCheckout ? <>{paymentPreviewLoading && <p className="text-sm text-gray-500">{t('payment.loadingQr')}</p>}{paymentPreview && <div className="rounded-2xl border border-forest-200 bg-forest-50/40 p-5 text-center"><p className="text-sm font-semibold text-gray-700 mb-2">{t('payment.amountToPay')}</p><p className="text-3xl font-bold text-forest-800 mb-5">RM{formatCurrency(total)}</p><div className="mx-auto w-fit rounded-2xl bg-white border border-cream-200 p-3 shadow-sm"><img src={paymentQrPublicUrl(paymentPreview.qrStoragePath)} alt="FreshGo DuitNow QR" className="w-64 max-w-full aspect-square object-contain" /></div><p className="mt-4 font-semibold text-forest-800">{t('payment.scanDuitNow')}</p><p className="mt-2 text-sm text-gray-600 leading-relaxed max-w-lg mx-auto">{paymentPreview.instructions || t('payment.checkoutQrInstructions')}</p></div>}{!paymentPreviewLoading && !paymentPreview && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">{paymentPreviewError || t('payment.qrUnavailable')}</div>}<div className="bg-jade-50 p-4 rounded-xl text-sm"><Lock size={15} className="inline mr-2"/>{t('payment.placeThenUpload')}</div></> : <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><Lock size={15} className="inline mr-2"/>{t('payment.weighedOrderInstructions')}</div>}<Totals/><div className="flex gap-3"><button className="btn-secondary flex-1" onClick={() => setStep('review')}>{t('checkout.back')}</button><button disabled={placing || paymentPreviewLoading || (priceFinalAtCheckout && !paymentPreview)} className="btn-primary flex-1" onClick={createOrder}>{placing ? t('checkout.placingOrder') : t('checkout.placeOrder', { total: formatCurrency(total) })}</button></div>{placeError && <p className="text-red-600 text-sm">{placeError}</p>}</div>;
