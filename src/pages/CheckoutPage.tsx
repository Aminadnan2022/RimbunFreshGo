...
import type { CustomerDetails, DeliveryDay, CartItem, ComboExpandedItem } from '../types';
...
const comboComponentQuantity = (component: ComboExpandedItem, comboQuantity: number) => {
  const amount = (component.quantityValue ?? component.quantity) * comboQuantity;
  if (component.sellingUnit === 'kg' || component.pricingType === 'per_kg') return `${amount}kg`;
...
useEffect(() => {
  ...
  const reviewAnswer = (question: PreparationQuestion, value: unknown) => {
    if (value === undefined || value === null || value === '') return '—';
    if (typeof value === 'boolean') return value ? t('checkout.yes') : t('checkout.no');

    const optionLabel = (answer: unknown) => {
      const option = question.options.find((candidate) => candidate.code === answer || candidate.value === answer);
      return option ? display(option) : String(answer);
    };

    return Array.isArray(value) ? value.map(optionLabel).join(', ') : optionLabel(value);
  };
  const reviewTargetLabel = (target: PreparationTarget, unit: number | null) =>
    unit !== null && target.quantity > 1 ? `${target.name} #${unit + 1}` : target.name;
  const reviewQuestionText = (target: PreparationTarget, unit: number | null, scope: 'line' | 'physical_unit') =>
    target.questionnaire.questions
      .filter((question) => question.selection_scope === scope)
      .flatMap((question) => {
        const value = answers[answerKey(target, unit)]?.[question.code];
        return value === undefined || value === null || value === ''
          ? []
          : [`${display(question)}: ${reviewAnswer(question, value)}`];
      })
      .join(' · ');
...
const Preparation = () => (
  ...
  const Review = () => <div className="card p-5 sm:p-8 space-y-5"><h2 className="font-semibold text-lg">{t('checkout.reviewTitle')}</h2>{cart.items.map((item, index) => { const lineKey = `line-${index}`; const lineTargets = targets.filter((target) => target.lineKey === lineKey); return <div key={index} className="border-b pb-3">{item.isCombo && item.comboItems?.length ? <div className="space-y-2">{item.comboItems.map((component, componentIndex) => { const componentNumber = component.componentNumber ?? componentIndex + 1; const target = lineTargets.find((candidate) => candidate.componentNumber === componentNumber); const lineText = target ? reviewQuestionText(target, null, 'line') : ''; const unitTexts = target ? Array.from({ length: target.quantity }, (_, unit) => ({ label: reviewTargetLabel(target, unit), text: reviewQuestionText(target, unit, 'physical_unit') })).filter((entry) => entry.text) : []; const hasPreparation = !!lineText || unitTexts.length > 0; return <div key={`${componentNumber}-${component.productId}`} className="text-sm text-gray-600">{!hasPreparation && <p><span className="font-medium text-gray-900">{component.name}</span> — {comboComponentQuantity(component, item.quantity)}</p>}{lineText && <p><span className="font-medium text-gray-900">{component.name}</span> — {lineText}</p>}{unitTexts.map((entry, unit) => <p key={unit}><span className="font-medium text-gray-900">{entry.label}</span> — {entry.text}</p>)}</div>; })}</div> : <><p className="font-medium">{item.name} · {weighted(item) ? `~${item.estimatedWeight ?? 0} kg` : t('checkout.qty', { count: item.quantity })}</p>{lineTargets.map((target) => { const lineText = reviewQuestionText(target, null, 'line'); const unitTexts = Array.from({ length: target.quantity }, (_, unit) => ({ label: reviewTargetLabel(target, unit), text: reviewQuestionText(target, unit, 'physical_unit') })).filter((entry) => entry.text); return <div key={target.key} className="ml-3 mt-2 text-sm text-gray-600">{lineText && <p>{reviewTargetLabel(target, null)} — {lineText}</p>}{unitTexts.map((entry, unit) => <p key={unit}>{entry.label} — {entry.text}</p>)}</div>; })}</>}</div>; })}<div className="rounded-xl bg-cream-50 p-4 text-sm"><p className="font-semibold">{details.name}</p><p>{details.houseUnit}, {details.deliveryPointName}</p><p>{details.phone} · {details.email}</p><p>{nextDate(deliveryDay!)} · {config.time}</p></div><Totals/><p className="text-xs text-amber-700">{t('checkout.finalPriceAfterWeighing')}</p><div className="flex gap-3"><button className="btn-secondary flex-1" onClick={() => setStep('preparation')}>{t('checkout.back')}</button><button className="btn-primary flex-1" onClick={() => setStep('payment')}>{t('checkout.continueToPayment')}</button></div></div>;
