...
useEffect(() => {
  ...
  const createOrder = async () => { if (!settings.allow_customer_orders) return setPlaceError(t('checkout.validation.ordersDisabled')); if (placementLock.current) return; placementLock.current = true; setPlacing(true); setPlaceError(null); try { const request = buildCanonicalPlaceOrderRequest({ customer: details, items: cart.items, deliveryMethod, deliveryDay, instantDate, instantTime, preparationTargets: targets, preparationAnswers: answers }); const order = await placeCanonicalOrder(request); await supabase.from('customer_profiles').upsert({ id: user!.id, email_address: user!.email, full_name: details.name, phone: details.phone, apartment: details.apartment, house_unit: details.houseUnit, pickup_location: details.pickupLocation, notes: details.notes || null, updated_at: new Date().toISOString() }, { onConflict: 'id' }); clearCart(); navigate(`/order/${order.order_number}`); } catch (err) { setPlaceError(err instanceof Error ? err.message : t('checkout.validation.failedToPlaceOrder')); } finally { placementLock.current = false; setPlacing(false); } };
